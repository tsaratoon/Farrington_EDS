# Function simulates epidemic curve outbreak data for the EDS
...
epidemicCurveOutbreak <- function(
  nData = 462,
  outbreakLength = "long",
  endPreOutbreak = 182,
  endOutbreak = 282,
  ...
  outbreakDist <- 4 * atanh(2*sample - 1)
  
  if (outbreakLength == "short") {
    outbreakHist <- hist(floor(2*outbreakDist), right=FALSE, plot=FALSE)$counts
  } else {
    outbreakHist <- hist(floor(2*outbreakDist), right=FALSE, plot=FALSE)$counts
  }
